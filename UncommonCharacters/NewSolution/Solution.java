...
public class Solution {
	...
	public static void applyPreOrder(BinaryTreeNode root, int hDistance, int level, HashMap<Integer, Pair> visited) {
		...
		if (!visited.containsKey(hDistance)) {
			Pair x = visited.get(hDistance);
			Pair temp = new Pair(root, level);
			visited.put(hDistance, temp);

		} else {
			Pair x = visited.get(hDistance);
			if (x != null && x.level > level) {
				x.root = root;
//				System.out.println(" i am heree..." + root.val + " " + hDistance + "  " + level);
				visited.put(hDistance, x);
			}
